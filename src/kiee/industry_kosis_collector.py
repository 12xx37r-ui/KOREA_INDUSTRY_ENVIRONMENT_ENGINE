...
def collect(root: Path, force: bool = False) -> dict[str, Any]:
    ...
    # 광공업(mining+manufacturing) 통계이며 금융·서비스·유틸리티 등은 이
    # 통계 범위 밖이다. 임의로 라벨을 매칭 시도해 우연히 잘못된 행에
    # 걸리거나 항상 빈 결과를 만드는 대신, 명시적으로 건너뛰고 그 사실을
    # 진단로그에 정직하게 남긴다. (미수집 데이터를 임의로 생성/추정 금지)
    non_manufacturing_scope = set(
        (config.get("non_manufacturing_scope") or {}).get("industries") or []
    )
